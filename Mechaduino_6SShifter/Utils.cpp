...
//Main motor controlling function, given rotational angle and PWM angle/phase
void output(float theta, int effort) {
  ...
  if (countItr>itrMax){
    uAverage = uAverage/itrMax;
    //SerialUSB.print(round(adcAverage),0);
    //SerialUSB.print("\t");    
    // SerialUSB.print(-r/10);
    // SerialUSB.print("\t");    
    // SerialUSB.println(uAverage);
  ...
  if (Serial1.available()) {
    ...
    switch (inChar) {
      ...
      case 'g':             //new setpoint
        ...
        gearAngle = Serial1.parseFloat();
        r = - (gearAngle*gearRatio) + magneticOffset;
        SerialUSB.println(r);
        break;
      default:
        break;
...
void serialMenu() {
  ...
  SerialUSB.println("");
  SerialUSB.println(" <  -  Gear down");
  SerialUSB.println(" >  -  Gear up");
  SerialUSB.println("");
