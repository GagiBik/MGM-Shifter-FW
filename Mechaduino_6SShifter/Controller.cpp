...
void TC5_Handler() {                              // gets called with FPID frequency
  ...
  if (TC5->COUNT16.INTFLAG.bit.OVF == 1) {        // A counter overflow caused the interrupt
    ...
    DTerm = pLPFa*DTerm -  pLPFb*pKd*(yw-yw_1);   // Integral term is not used/usefull.
    u = (pKp * e) + DTerm; 

    y_1 = y;  //copy current value of y to previous value (y_1) for next control cycle before PA angle added

    if (u > 0){       //Depending on direction we want to apply torque, add or subtract a phase angle of PA for max effective torque.  
      ...
      if (u > uMAX) u = uMAX;       //limit control effort saturation limits max current command
    }
    else {
      y -= PA;          //update phase excitation angle
    ...
    if (abs(e) < 360) ledPin_HIGH();    // turn on LED if error is less than 0.1
